...
{
   public class MeasurementRepository : IMeasurementUnitRepo
   {
      private readonly RavenDBContext _db;
      public MeasurementRepository(RavenDBContext db)
      {
         _db = db;
      }
      public SaveVM SaveMeasurementUnit(MeasurementUnitVM obj)
      {
         try
         {
            ...
            var result = _db.Database.SqlQuery<SaveVM>("InsertUpdateMeasurementUnit_SP  @ID,@Name,@Description,@IsActive,@Creator",
                ID, Name, Description,isActive, Creator).FirstOrDefault();
      ...
      public IEnumerable<MeasurementUnitVM> GetMeasurement(long Id)
      {
         var parms = new SqlParameter("@ID", Id); 
         var result = _db.Database.SqlQuery<MeasurementUnitVM>("GetMeasurementUnit_SP @ID", parms).ToList();
         return result;
      }
   }
}
